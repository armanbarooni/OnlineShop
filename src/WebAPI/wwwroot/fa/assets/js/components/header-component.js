...
class HeaderComponent {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const allCategoriesResult =
          await window.categoryService.getAllCategories();
  ...
  renderCategoryMenu(categories) {
    ...
    const mainNavContainer = document.querySelector(
      "[data-category-menu-main]",
    );
    if (
      mainNavContainer &&
      ...
      const html = limitedCategories
        .map((category) => this.renderCategoryMenuItem(category, true))
        .join("");
      mainNavContainer.innerHTML = html;
    }

    // Offcanvas mobile menu
    const offcanvasMenuContainer = document.querySelector(
      "[data-category-menu-offcanvas]",
    );
    if (
      ...
      const html = categories
        .map((category) => this.renderOffcanvasCategoryItem(category))
        .join("");
    ...
    const megaList =
      document.querySelector("#mega-menu-fire-target ul") ||
      document.querySelector("#mega-menu-fire-target .col-span-2 ul");
