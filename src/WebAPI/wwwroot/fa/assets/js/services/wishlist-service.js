...
class WishlistService {
  ...
  async searchWishlist(query) {
    try {
      const response = await this.apiClient.get(
        `/api/Wishlist/search?q=${encodeURIComponent(query)}`,
      );
      return {
        success: true,
        data: response.data || response,
  ...
  async getWishlistByCategory(categoryId) {
    try {
      const response = await this.apiClient.get(
        `/api/Wishlist/category/${categoryId}`,
      );
      return {
        success: true,
        data: response.data || response,
  ...
  async sortWishlist(sortBy, sortOrder = "asc") {
    try {
      const response = await this.apiClient.get(
        `/api/Wishlist/sort?sortBy=${sortBy}&sortOrder=${sortOrder}`,
      );
      return {
        success: true,
        data: response.data || response,
  ...
  async getWishlistPaginated(pageNumber = 1, pageSize = 10) {
    try {
      const response = await this.apiClient.get(
        `/api/Wishlist?pageNumber=${pageNumber}&pageSize=${pageSize}`,
      );
      return {
        success: true,
        data: response.data || response,
  ...
  isProductAvailableInWishlist(wishlist, productId) {
    ...
    return (
      item && item.product && item.product.isAvailable && item.product.isInStock
    );
  }
