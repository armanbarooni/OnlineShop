...
// Create product card HTML - List View (Full Width)
function createProductCard(product) {
    ...
    const description = product.description || 'توضیحات محصول برای این کالا ثبت نشده است.';

    return `
        <div class="col-span-12 w-full mb-4">
            <article class="bg-white product-box-item drop-shadow-md rounded-xl p-4 dark:bg-gray-800 dark:border-white dark:border-1 flex md:flex-row flex-col gap-4 items-center">
                <!-- Image Section -->
                <div class="md:w-1/4 w-full relative">
                    ...
                    <a href="${productUrl}" class="block">
                        <figure class="overflow-hidden rounded-lg">
                            <img src="${imageUrl}" alt="${name}" class="w-full h-40 object-contain hover:scale-105 transition" onerror="this.src='assets/images/product/mobile-1.png'">
                        </figure>
                    </a>
                </div>

                <!-- Info Section -->
                <div class="md:w-2/4 w-full flex flex-col justify-start text-start h-full">
                    <a href="${productUrl}">
                        <h3 class="text-lg font-bold mb-2 dark:text-white">${name}</h3>
                    </a>
                    <p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-3">${description}</p>
                    <div class="flex items-center gap-2">
                         <button onclick="addToWishlist('${product.id}')" class="p-2 bg-gray-100 dark:bg-gray-700 rounded-full hover:bg-primary hover:text-white transition">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/>
                            </svg>
                        </button>
                        <span class="text-xs text-gray-400">افزودن به لیست علاقه‌مندی‌ها</span>
                    </div>
                </div>

                <!-- Price & Action Section -->
                <div class="md:w-1/4 w-full flex flex-col items-center md:items-end justify-between border-s border-gray-100 dark:border-gray-700 md:ps-4 gap-4">
                    <div class="flex flex-col items-center md:items-end">
                        ${discount > 0 ? `<span class="text-xs text-gray-400 line-through">${formatPrice(price)}</span>` : ''}
                        <span class="text-xl font-bold text-primary">${formatPrice(finalPrice)} تومان</span>
                    </div>
                    <button onclick="addToCart('${product.id}')" class="w-full bg-primary text-white py-2 px-4 rounded-lg hover:bg-primary/90 transition flex items-center justify-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
